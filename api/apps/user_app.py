...
def talkweb_callback():
    ...
    res = requests.get(
        ...
        params={
            "client_id": settings.TALKWEB_OAUTH.get("client_id"),
            "client_secret": settings.TALKWEB_OAUTH.get("secret_key"),
            "grant_type": settings.TALKWEB_OAUTH.get("grant_type"),
    ...
    session["access_token_from"] = "talkweb"
    user_info = user_info_from_talkweb(session["access_token"])
    email_address = user_info["email"]
    users = UserService.query(email=email_address)
    user_id = user_info["uniqueNo"]
    if user_id is None:
        user_id = get_uuid()
    ...
    if not users:
        ...
        try:
            try:
                ...
                logging.exception(e)
                avatar = ""
            # {'uniqueNo': '1428335833338306561', 'mobile': '15084869636',
            #  'avatar': 'http://itdev.talkweb.com.cn/api/file-server/file/download?fileId=group1/M00/00/00/CvQEX2aY4X2AJQe_AACaiQH5wFU656.png',
            #  'userId': '1676083559315587074', 'user': '21949', 'email': 'liqiao@talkweb.com.cn',
            #  'username': '21949', 'realname': '李巧'}
            users = user_register(
            user_id,
            {
                ...
                "email": email_address,
                "avatar": avatar,
                "nickname": f'({user_info["realname"]}({user_info["username"]}))',
                "login_channel": "talkweb",
...
def user_info_from_talkweb(access_token):
    import requests
